...
export const getGlobalStats = async (_req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const soldTickets = await RequestModel.countDocuments({ is_dispatched: true });
    const pendingTickets = await RequestModel.countDocuments({ status: "Pending" });
    const inProgressTickets = await RequestModel.countDocuments({ status: "In-Transit" });
    const completedTickets = await RequestModel.countDocuments({ status: "Verified" });
    const onHoldTickets = await RequestModel.countDocuments({ status: "Damaged" });

    const requests = await RequestModel.find();
    const totalRevenue = requests.reduce((sum: number, r: any) => sum + (r.total_value || 0), 0);
  ...
  try {
    ...
    const userStats = await Promise.all(
      users.map(async (user: any) => ({
        ...
        completedTickets: await RequestModel.countDocuments({ user_id: user.id, status: "Verified" }),
  ...
  try {
    ...
    const searchFilter = {
      $or: [
        ...
        { entity_phone: { $regex: query, $options: "i" } },
        { entity_email: { $regex: query, $options: "i" } },
        { product_name: { $regex: query, $options: "i" } },
        { batch_sku: { $regex: query, $options: "i" } },
...
export const getActivityLogs = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const logs = await RequestModel.find()
      ...
      .limit(limitNum)
      .skip(offsetNum)
      .select("user_id entity_name status created_at updated_at");
    ...
    res.json({
      data: logs.map((log: any) => ({
        ...
        resource: `Record: ${log.entity_name}`,
        status: log.status,
