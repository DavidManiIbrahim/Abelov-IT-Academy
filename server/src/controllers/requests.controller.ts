...
export const getStats = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const stats = {
      ...
      inTransit: requests.filter((r: any) => r.status === "In-Transit").length,
      damaged: requests.filter((r: any) => r.status === "Damaged").length,
      totalRevenue: requests.reduce((sum: number, r: any) => sum + (r.total_value || 0), 0),
    };
