...
export default function AnalyticsDashboard() {
  ...
  const revenueOverTime = requests.reduce((acc, req) => {
    ...
    if (existing) {
      existing.revenue += req.transaction_completed ? (req.total_value || 0) : (req.amount_paid || 0);
      existing.count += 1;
    } else {
      acc.push({ month, revenue: req.transaction_completed ? (req.total_value || 0) : (req.amount_paid || 0), count: 1 });
    }
    return acc;
  ...
  const deviceBreakdown = requests.reduce((acc, req) => {
    const device = req.product_name || 'Unknown';
  ...
  // Status trends
  const statusTrends = [
    { status: 'Sold', count: requests.filter(r => r.status === 'Sold').length },
    { status: 'Verified', count: requests.filter(r => r.status === 'Verified').length },
    ...
    { status: 'Damaged', count: requests.filter(r => r.status === 'Damaged').length },
  ].filter(item => item.count > 0);

  // Recorder work breakdown
  const technicianWork = requests.reduce((acc, req) => {
    const tech = req.recorder_name || 'Unassigned';
    const existing = acc.find(item => item.technician === tech);
    if (existing) {
      existing.completed += req.status === 'Verified' ? 1 : 0;
      existing.inProgress += req.status === 'In-Transit' ? 1 : 0;
      existing.pending += req.status === 'Pending' ? 1 : 0;
      existing.onHold += req.status === 'Damaged' ? 1 : 0;
      existing.total += 1;
    } else {
      acc.push({
        technician: tech,
        completed: req.status === 'Verified' ? 1 : 0,
        inProgress: req.status === 'In-Transit' ? 1 : 0,
        pending: req.status === 'Pending' ? 1 : 0,
        onHold: req.status === 'Damaged' ? 1 : 0,
        total: 1,
      });
    }
    return acc;
  ...
  const totalRevenue = requests.reduce((sum, r) => sum + (r.transaction_completed ? (r.total_value || 0) : (r.amount_paid || 0)), 0);
  const avgServiceTime = requests.length > 0 ? Math.round(requests.reduce((sum, r) => {
    const created = new Date(r.created_at);
    // Use updated_at if status is verified, otherwise use current time for duration calculation
    const end = r.status === 'Verified' ? new Date(r.updated_at) : new Date();
